...
public class MyJsonType implements UserType {
	...
	@Override
	public void nullSafeSet(final PreparedStatement ps, final Object value,
			final int idx, final SessionImplementor session)
			throws HibernateException, SQLException {
		if (value == null) {
		...
		try {
			...
			w.flush();
			ps.setObject(idx, w.toString(), Types.BLOB);
		} catch (final Exception ex) {
